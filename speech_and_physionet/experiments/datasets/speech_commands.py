...
def _process_data(intensity_data):
    ...
    for foldername in ('yes', 'no', 'up', 'down', 'left', 'right', 'on', 'off', 'stop', 'go'):
        ...
        for filename in os.listdir(loc):
            # fix to support more recent torchaudio version
            if version.parse(torchaudio.__version__) < version.parse("0.9.0"):
                audio, _ = torchaudio.load_wav(loc / filename, channels_first=False,
                                               normalization=False)
            else:
                audio, _ = torchaudio.load(loc / filename, channels_first=False,
                                           normalize=False)
            audio = audio / 2 ** 15  # manual normalization; apparently behavior depends on version (to be checked).
...
def get_data(intensity_data, batch_size, loginf=None):
    ...
    if os.path.exists(loc):
        ...
        if loginf is not None:
            loginf("Downloading/Processing data...")
        download()
        (times, train_coeffs, val_coeffs, test_coeffs, train_y, val_y, test_y, train_final_index, val_final_index,
