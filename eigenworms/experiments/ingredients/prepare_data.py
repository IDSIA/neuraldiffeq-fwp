...
def ready_all_data_and_model(model_type,
                             depth,
                             step,
                             hidden_dim,
    ...
    # Setup as datasets
    train_ds, train_sampler = build_dataset(
        model_type=model_type, data=train_data, depth=depth, step=step,
        sampler_name=sampler_name)
    val_ds, val_sampler = build_dataset(
        model_type=model_type, data=val_data, depth=depth, step=step,
        sampler_name=sampler_name)
    test_ds, test_sampler = build_dataset(
        model_type=model_type, data=test_data, depth=depth, step=step,
        sampler_name=sampler_name)

    # Setup as datasets
    train_dl = build_dataloader(train_ds, sampler=train_sampler, batch_size = batch_size)
    val_dl = build_dataloader(val_ds, sampler=val_sampler, batch_size = batch_size)
    test_dl = build_dataloader(test_ds, sampler=test_sampler, batch_size = batch_size)

    # Some params
    input_dim = train_ds.input_dim
    if model_type == 'nrde' or 'fwp' in model_type:
        initial_dim = train_ds.initial_dim
    else:
        initial_dim = None
    ...
    if not ignore_model:
        model, n_params = get_model(
            ...
            trafo_ff_dim=trafo_ff_dim, num_heads=num_heads, learning_rule=learning_rule,
            scale_out_proj=scale_out_proj, scale_ff=scale_ff,
            scale_query_net=scale_query_net, residual=residual,
...
def process_data(ds_folder='UEA', ds_name='EigenWorms', missing_rate=None, include_observational_intensity=False, imputation_method='ffill',
                 train_test_val_seed=0, val_frac=0.15, test_frac=0.15, scaling='stdsc', model_type=None):
...
def build_dataset(model_type, sampler_name, data, depth, step):
    ...
    if model_type in ['logsig-rnn', 'nrde', 'logsig-ode-fwp', 'logsig-cde-fwp', 'multi-logsig-ode-fwp']:
...
def prepare_additional_channels(controls, model_type, include_observational_intensity):
    ...
    if model_type in ['gru-d', 'gru-dt']:
        controls = prepare_gru_variant_data(controls, variant=model_type, intensity=include_observational_intensity)
         ...
         if model_type == ['nrde', 'logsig-rnn', 'logsig-ode-fwp', 'logsig-cde-fwp']:
