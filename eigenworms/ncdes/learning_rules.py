...
class HebbUpdateVectorField(torch.nn.Module):
    ...
    def forward(self, x, W=None):  # Hebbian rule does not depend on current value of W
        ...
        vs = lrs * torch.tanh(vs)  # just to try TODO change
...
class DeltaUpdateVectorField(torch.nn.Module):
    def __init__(self, input_channels, hidden_channels, num_heads,
                 post_tahn=False, skip_input_proj=False):
...
class CDEDeltaUpdateVectorField(torch.nn.Module):
    ...
    def forward(self, x, dx_dt, W=None):
        ...
        lrs = torch.sigmoid(lrs)
        ks = torch.softmax(ks, dim=-1)
        vs = torch.tanh(vs)  # just to try TODO change
...
class DxOnlyCDEDeltaUpdateVectorField(torch.nn.Module):
    ...
    def forward(self, x, dx_dt, W=None):
        ...
        # tanh below seems to be ultra crucial for long span ODEs
        if W is not None:
            # Delta rule:
            W_t = W.reshape(bsz, self.num_heads, self.head_dim, self.head_dim)
            W_t = W_t.reshape(bsz * self.num_heads, self.head_dim, self.head_dim)
            vs_remove = torch.bmm(W_t, ks.unsqueeze(2)).squeeze()
            vs = lrs * torch.tanh(vs - vs_remove)
        else:
...
# hard code 2-layer case
class MultiLayerLearningRuleVectorFieldWrapper(torch.nn.Module):
    def __init__(self, X, rule_func, rule_func2, ff_block, query_net,
                 state_size, num_heads, head_dim, residual=False):
        """Generic wrapper for learning rule vector field for ODE.
        ...
        Arguments:
            ...
            rule_func: learning rule function.
        """
        super(MultiLayerLearningRuleVectorFieldWrapper, self).__init__()
        self.X = X
        self.rule_func = rule_func
        self.rule_func2 = rule_func2
        self.ff_block = ff_block
        self.mid_query_net = query_net
    ...
    def __call__(self, t, z):
        # split z
        z1, z2 = torch.split(z, [self.state_size, self.state_size], dim=-1)
        # first layer
        # control_gradient is of shape (..., input_channels)
        control_x = self.X[t]
        ...
        # vector_field is of shape (..., hidden_channels, input_channels)
        out1 = self.rule_func(control_x, z1)
        # z_t is a weight matrix, but needs to be reshaped to be so
        out2 = out1.reshape(bsz, self.num_heads, self.head_dim, self.head_dim)
        out2 = out2.reshape(bsz * self.num_heads, self.head_dim, self.head_dim)

        qs_prel = self.rule_func.input_proj(control_x)
        qs = self.rule_func.input_layer_norm(qs_prel)
        ...
        # 'bij, bj->bi'
        out2 = torch.bmm(out2, qs.unsqueeze(2)).squeeze()
        out2 = out2.reshape(bsz, self.num_heads, self.head_dim)
        out2 = out2.reshape(bsz, self.num_heads * self.head_dim)
        if self.residual:
            out2 = out2 + qs_prel
    
        out2 = self.ff_block(out2)

        # second layer
        out2 = self.rule_func2(out2, z2)
